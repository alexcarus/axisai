// ---------------------------------------------------------------------------
// AXIS AI — swaps on Robinhood Chain (ETH ⇄ AXIS).  LIVE.
//
// Mirrors ../lib/uniswap-v4.ts (Base) but targets Robinhood Chain and the
// bridged AXIS (the OFT). The ETH/AXIS Uniswap v4 pool is live on Robinhood
// Chain (poolId derived below) and the DEX addresses are explorer-verified, so
// this trades against the real pool. `isConfigured()` guards every call and is
// true only while those addresses are set, so a misconfig can never hit a wrong
// pool.
// ---------------------------------------------------------------------------
...
// Uniswap v4 on Robinhood Chain — from the official deployments, cross-checked
// on-chain (code present + StateView/Quoter functional) and against the live
// pool-creation tx (PoolManager/PositionManager matched). Verified 2026-07-13.
...
export const POOL_ID = keccak256(
  encodeAbiParameters(
    [{ type: "tuple", components: POOLKEY_COMPONENTS }],
    [POOL_KEY],
  ),
) as Hex;
...
export async function ensureAllowances(
  ...
  const erc = (await publicClient.readContract({
    address: inputToken,
    abi: ERC20_ABI,
