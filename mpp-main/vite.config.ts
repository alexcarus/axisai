...
// Resolve build provenance from git when available. Falls back gracefully when
// building outside a git checkout (e.g. a downloaded source copy) so the config
// can still load and the site can build/dev.
function git(cmd: string, fallback: string): string {
  try {
    return child_process.execSync(cmd, { stdio: ["ignore", "pipe", "ignore"] })
      .toString()
      .trim();
  } catch {
    return fallback;
...
const commitSha = git("git rev-parse --short HEAD", "nogit");
const commitTimestamp = git("git log -1 --format=%cI", new Date().toISOString());
